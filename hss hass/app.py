...
# ✅ UPDATED: use your new MODEL-DRIVEN pipeline (KMeans + RF + LR, no GMM)
# In your analysis_module.py, export the function below with this exact name.
...
def home():
    if current_user.role == "cto":
        return redirect(url_for("cto_dashboard"))
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password, password):
            if user.role == "manager" and not user.is_approved:
                flash("Your account is still pending CTO approval.")
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            db.session.commit()
            flash("Account created! Access is pending CTO approval.")
            return redirect(url_for("login"))
        except Exception:
            db.session.rollback()
...
def cto_dashboard():
    ...
    return render_template("cto_dashboard.html", pending=pending, hires=hires, rejections=rejections)
...
def manager_dashboard():
    ...
    # ✅ UPDATED: pulls model-driven profiles output
    # Expected output format: {"GitHub":[...], "StackOverflow":[...], "Kaggle":[...]}
    ...
    if request.method == "POST":
        ...
        role = request.form.get("role")  # could be "Profile A/B/C" or whatever you display
...
with app.app_context():
    ...
    # Create Default CTO if doesn't exist
    if not User.query.filter_by(username="cto").first():
        cto = User(
            username="cto",
            email="cto@admin.com",
